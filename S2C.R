# The code below was written by Finn Lindgren, see
# https://github.com/finnlindgren/spectral-notes
...
S2sample <- function(S, dim, h, seed = NULL) {
  ...
  if(length(dim)==1){
    ...
    sample <- Re(fft(matrix(rnorm(prod(dim)) + 1i*rnorm(prod(dim)),dim[1],dim[2]) * SD))
  }
  sample
}
